...
export async function generateMetadata(props: {
  ...
  return {
    title: `${LANGUAGE_NAME} ${level.toUpperCase()} — Review questions — Languages Center`,
  };
}
          ...
          <div className="flex items-center justify-between gap-3 mb-6">
            <div>
              <p className="text-xs font-semibold text-teal-dark uppercase tracking-wider mb-1">
                {LANGUAGE_NAME} {LEVEL} Placement Exam — Review
              </p>
  ...
  return (
    <div className={`border-2 rounded-2xl p-5 ${borderClass}`}>
      ...
      <p className="text-xs text-navy/60 mb-3">
        You played <span className="font-semibold">{rp.user_role}</span> · AI
        played <span className="font-semibold">{rp.ai_role}</span>
      </p>
  ...
  return (
    <div className={`border-2 rounded-2xl p-5 ${borderClass}`}>
      <div className="flex items-start justify-between gap-3 mb-2">
        ...
        <span
          className={`text-xs font-bold px-2 py-0.5 rounded-full ${
            correct ? "bg-teal text-white" : "bg-amber-600 text-white"
          }`}
        >
          {item.score}/10
        </span>
